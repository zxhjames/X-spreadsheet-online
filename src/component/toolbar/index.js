...
function moreResize() {
  ...
  el.css('width', `${this.widthFn() - 60}px`);//60
  const elBox = el.box();
  
  //MODIFIED: 修改 sumWidth 控制 more 按钮的缩放行为，目前已符合响应式布局的需求
  let sumWidth = 480;
  let sumWidth2 = 12;
  const list1 = [];
  const list2 = [];
...
export default class Toolbar {
  constructor(data, widthFn, isHide = false) {
    ...
    //   ],
      ...
      buildDivider(),
      [
          this.sheetLoaderEl = new sheetLoader(),
          this.sheetUpdaterEl = new sheetUpdater();
          this.fieldLoaderEl = new fieldLoader();
          this.fieldListEl = new fieldList();
      ],
      */
    ];
